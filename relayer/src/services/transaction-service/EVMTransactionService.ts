  ...
  implements ITransactionService<IEVMAccount, EVMRawTransactionType>
{
  ...
  private async createTransaction(
    ...
    const { from, to, value, data, gasLimit, speed, account } =
    ...
    if (typeof gasPrice !== 'string') {
      log.info(
        `Gas price being used to send transaction by relayer: ${relayerAddress} is: ${JSON.stringify(
          gasPrice
        )}`
      );
    ...
    log.info(
      `Gas price being used to send transaction by relayer: ${relayerAddress} is: ${gasPrice}`
    );
  ...
  async executeTransaction(
    ...
    try {
      const transactionExecutionResponse =
      ...
      const nonceErrorMessage =
        config.transaction.errors.networksNonceError[this.chainId];
      const replacementFeeLowMessage =
        config.transaction.errors.networkResponseMessages
          .REPLACEMENT_UNDERPRICED;
      const alreadyKnownMessage =
        config.transaction.errors.networkResponseMessages.ALREADY_KNOWN;
      const insufficientFundsErrorMessage =
        config.transaction.errors.networksInsufficientFundsError[
          this.chainId
        ] ||
        config.transaction.errors.networkResponseMessages.INSUFFICIENT_FUNDS;

      if (
        errInString.indexOf(nonceErrorMessage) > -1 ||
        errInString.indexOf(
          'increasing the gas price or incrementing the nonce'
        ) > -1
      ) {
        log.info(
    ...
    on network id ${this.chainId}. Removing nonce from cache and retrying`
        ...
        rawTransaction.nonce = await this.networkService.getNonce(
          rawTransaction.from,
          true
        );
     ...
     on network id ${this.chainId}`
        ...
        log.info(
          `rawTransaction.gasPrice ${rawTransaction.gasPrice} for relayer ${rawTransaction.from} on network id ${this.chainId}`
        );
        ...
        log.info(
          `transaction sent with gas price ${rawTransaction.gasPrice} for relayer ${rawTransaction.from} on network id ${this.chainId}`
        );
        log.info(
          `Bumping up gas price with multiplier ${
            config.transaction.bumpGasPriceMultiplier[this.chainId]
          } for relayer ${rawTransaction.from} on network id ${this.chainId}`
        );
        log.info(
          `gasPriceInNumber ${gasPriceInNumber} for relayer ${rawTransaction.from} on network id ${this.chainId}`
        );
        rawTransaction.gasPrice = Math.round(
          config.transaction.bumpGasPriceMultiplier[this.chainId] *
            gasPriceInNumber
        ).toString();
        log.info(
          `increasing gas price for the resubmit transaction ${rawTransaction.gasPrice} for relayer ${rawTransaction.from} on network id ${this.chainId}`
        );
  ...
  async sendTransaction(
    ...
    if (retryTransactionCount > maxRetryCount) {
      try {
        ...
        console.log('send retry attempts exceeded notification');
      } catch (error) {
        log.error(error);
    ...
    log.info(
      `Transaction request received with transactionId: ${transactionId} on chainId ${this.chainId}`
    );
    // create transaction
    ...
    log.info(
      `Raw transaction for transactionId: ${transactionId} is ${JSON.stringify(
        rawTransaction
    ...
    try {
      ...
      log.info(
        ...
        }: ${JSON.stringify(transactionExecutionResponse)} on chainId ${
          this.chainId
        }`
      );

      log.info(
        `Incrementing nonce for account: ${relayerAddress} on chainId ${this.chainId}`
      );
      await this.nonceManager.incrementNonce(relayerAddress);
      log.info(
        `Incremented nonce for account: ${relayerAddress} on chainId ${this.chainId}`
      );
      ...
      const transactionListenerNotifyResponse =
        await this.transactionListener.notify({
          transactionExecutionResponse:
            transactionExecutionResponse.transactionResponse,
          transactionId: transactionId as string,
          relayerAddress,
          transactionType,
          previousTransactionHash: undefined,
  ...
  async retryTransaction(
    ...
    try {
      ...
      if (retryTransactionExecutionResponse.success) {
        transactionExecutionResponse =
          retryTransactionExecutionResponse.transactionResponse;
      } else {
        return {
          state: 'failed',
          code: 500,
      ...
      log.info(
        `Error while retrying transaction: ${error} for transactionId: ${transactionId} on chainId: ${this.chainId}`
      );
      return {
        state: 'failed',
        code: 500,
