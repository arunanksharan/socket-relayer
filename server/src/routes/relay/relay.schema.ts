...
const userOp = object.keys({
  ...
  nonce: string
    .required()
    .error(new Error('nonce is required and should be a hex string')),
  initCode: string,
  callData: string
    .required()
  ...
  callGasLimit: string
    .required()
    .error(new Error('callGasLimit is required and should be a hex string')),
  verificationGasLimit: string
    .required()
    .error(
      new Error('verificationGasLimit is required and should be a hex string')
    ),
  preVerificationGas: string
    .required()
    .error(
      new Error('preVerificationGas is required and should be a hex string')
    ),
  maxFeePerGas: string
    .required()
    .error(new Error('maxFeePerGas is required and should be a hex string')),
  maxPriorityFeePerGas: string
    .required()
    .error(
      new Error('maxPriorityFeePerGas is required and should be a hex string')
    ),
...
export const aaRequestSchema = object.keys({
  ...
  params: array.items(
    alternatives.try(userOp, entryPointAddress, chainId, metaData)
  ),
...
export const crossChainRequestSchema = object.keys({
  ...
  params: object.keys({
    ...
    to: string
      .regex(/^0x[a-fA-F0-9]{40}$/)
      .required()
      .error(new Error('to address is required')),
    ...
    refundInfo: object
      .keys({
        tokenGasPrice: string
          .required()
          .error(new Error('tokenGasPrice is required')),
        gasToken: string
          .required()
          .error(new Error('gasToken type sent in request is required')),
      })
      .required()
      .error(new Error('refundInfo is required')),
  }),
  jsonrpc: string.required().error(new Error('jsonrpc is required')),
  id: number.required().error(new Error('id is required')),
});
